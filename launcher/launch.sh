...
create_window() {
    ...
    tmux send-keys -t $SESSION "source /opt/homebrew/Caskroom/miniforge/base/bin/activate $MAMBA_ENV && export PYTHONPATH=./launcher && $command" C-m
    sleep $sleep_time
}
...
create_window "celery-titration" \
    "celery -A task worker -Q titration --concurrency=1 --loglevel=INFO -E -n titration"

create_window "celery-titration-stitching" \
    "celery -A task worker -Q image_stitch_titration --concurrency=3 --loglevel=INFO -E -n image_stitcher_titration"
